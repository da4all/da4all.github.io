    ...
    <div class="card {% if card.inline == false %}hoverable{% endif %}">
        <div class="row no-gutters">
		    ...
		    {% if card.profile.group %}<h6 class="card-subtitle mb-2 text-muted">Type: {{ card.profile.group }}</h6>{% endif %}
                    {% if card.profile.source %}<h6 class="card-subtitle mb-2 text-muted">Literacy: {{ card.profile.source }}</h6>{% endif %}
                    {% if card.profile.author %}<h6 class="card-subtitle mb-2 text-muted">{{ card.profile.author }}</h6>{% endif %}
